...
esp_err_t anim_vfs_init(const uint8_t *data, size_t data_len, anim_vfs_handle_t *ret_parser)
{
    ...
    // ESP_LOGI(TAG, "HELLO!! data_len=%d ", data_len);

    // ESP_LOG_BUFFER_HEXDUMP("HELLO!! gif=", data, 4800, ESP_LOG_INFO);
    ...
    // ESP_LOGI(TAG, "HELLO!! total_frames=%d ", total_frames);
    ...
    for (int i = 0; i < total_frames; i++) {
        ...
        (entries + i)->asset_mem = (void *)(data + table[i].asset_offset);
        // ESP_LOGI(TAG, "HELLO!! i=%d table[%d].asset_offset=0x%0x size=%d sizeof(asset_table_entry_t)=%d", i, i, table[i].asset_offset, table[i].asset_size, sizeof(asset_table_entry_t));
