...
image_format_t anim_dec_parse_header(const uint8_t *data, size_t data_len, image_header_t *header)
{
    ...
    // ESP_LOG_BUFFER_HEXDUMP("HELLO!! image=", data, data_len, ESP_LOG_INFO);
    ...
    if (strncmp(header->format, "_S", 2) == 0) {
        ...
        // Validate bit depth
        if (header->bit_depth != 4 && header->bit_depth != 8 && header->bit_depth != 1) {
        ...
        /**
         * 这里按照aaf预先切片的规则，将aaf的数据从flash拷贝出来 存在SRAM里的数组中
         * 因为在生成aaf的时候，就已经将动画的一帧数据都进行了切片存储，每一个切片，都会往aaf文件的协议头的尾巴，添加该切片的颜色信息（占两个字节）。
         * example：
         * 比如，针对黑白的0.96寸OLED(width=128, height=64)，header->splits数值为8时，意味着高度上的像素需要均等分切成八份，
         * 也即将单列上的height=64个像素点，以8个像素为单位切为8份。这里就需要存8*2个字节的split_length信息到aaf文件的协议头尾巴里（统一存到aaf文件的第19个字节及之后的内存位置）
         * 
         */
        ...
        /**
         * 所以，对于OLED屏幕而言，要使用esp32的anim库，则对于0.96寸黑白屏OLED， bit_depth=8，
         * width=128， height=64,  splits=8（表示image数组里的一个字节表示几个像素点，这里表示8个像素点），
         * split_height=8（表示将height分成8份，每一份刚好是8，
         * 也即对应了splits的值，也就构成了heigth这个64的值）。
         */
        // Calculate number of colors based on bit depth
        header->num_colors = 1 << header->bit_depth;
        // ESP_LOGI(TAG, "HELLO!!!!  bit depth: %d width=%d height=%d splits=%d split_height=%d num_colors=%d", 
        //                 header->bit_depth, header->width, header->height, header->splits, header->split_height, header->num_colors);
        ...
        /**
         * palette datas是颜色信息，比如有些lcd屏幕是0~255个色彩。
         * 对于OLED来说，其实就2个色彩，也即 header->num_colors 应该等于2，所以这里生成的aaf文件的协议头里的 header->bit_depth 应当是1
         * 从这里可见， palette 颜色信息是保留在 (data + 18 + header->splits * 2)后边，信息长度为 像素位深度*4
         */
        // Read palette data
...
void anim_dec_calculate_offsets(const image_header_t *header, uint16_t *offsets)
{
    ...
    for (int i = 1; i < header->splits; i++) {
        ...
        ESP_LOGI(TAG,"HELLO!! offsets[%d] = %d, splits_length=%d", i, offsets[i], header->split_lengths[i - 1]);
    }
}
