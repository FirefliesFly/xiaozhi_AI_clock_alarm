/*depend on espressif2022/image_player: ==1.1.0~1 */   
...
static esp_err_t anim_player_parse(const uint8_t *data, size_t data_len, image_header_t *header, anim_player_context_t *ctx)
{
    ...
    // ESP_LOG_BUFFER_HEXDUMP("HELLO!! player=", data, data_len, ESP_LOG_INFO);
    ...
    if (header->bit_depth == 4) {
        ...
        //正是由于这里的限制，所以针对OLED黑白屏，要么增加bit_depth为1的case处理条件；要么bit depth只能为8，这会增大palette data占用的flash内存。
        decode_buffer = (uint8_t *)malloc(header->width * header->split_height);
    }
    else if(header->bit_depth == 1)
    {
        decode_buffer = (uint8_t *)malloc(header->width * (header->split_height + (header->split_height % 2)) / 8);
    }
    ...
    // pixels指向的内存空间，用来存储实际写入屏幕驱动的像素集合
    uint16_t *pixels = (uint16_t *)frame_buffer;

    //开始按照对每一帧切片的结果，进行逐片刷新屏幕，直至将一帧信息完全刷新进去为止
    // Process each split
    for (int split = 0; split < header->splits; split++) {
        //compressed_data指向的就是一帧中某一切片的动画数据（含压缩field的信息）
        ...
        // if (compressed_data[0] == ENCODING_TYPE_RLE) 
        {
            if(header->bit_depth == 1)
            {
                decode_result = anim_dec_rte_decode(compressed_data + 1, compressed_len - 1,
                                                    decode_buffer, header->width * header->split_height/8);
            }
            else
            {
                //这里decode_buffer已经不用指向动画帧信息（含编解码标识）的起始位置了，只需要指向切片实际动画的数据就行，这里就是让decode_buffer指向切片实际动画数据
                decode_result = anim_dec_rte_decode(compressed_data + 1, compressed_len - 1,
        ...
        if (header->bit_depth == 4) {
            ...
            //用列行式处理黑白OLED
            //1-bit mode: each byte is one pixel
            memcpy((uint8_t *)pixels, (uint8_t *)decode_buffer, header->width * header->split_height/8);
        }
        else
        {
...
static void anim_player_task(void *arg)
{
    ...
    while (1) {
        ...
        if (xQueueReceive(ctx->events.event_queue, &player_event, 0) == pdTRUE) {
            ...
            run_ctx.repeat = ctx->repeat;
            run_ctx.fps = ctx->fps;
            ESP_LOGI(TAG, "Player updated [%s]: %d -> %d, repeat:%d, fps:%d",
        ...
        do {
            ESP_LOGI(TAG, "HELLO!! anim player, run_start=%d, run_end=%d stop=%d fps=%d", run_ctx.run_start, run_ctx.run_end, run_ctx.action, run_ctx.fps);
            for (int i = run_ctx.run_start; (i <= run_ctx.run_end) && (run_ctx.action != PLAYER_ACTION_STOP); i++) {
                ...
                /**
                 * @param ctx->info 
                 *        ctx->info的赋值都是通过anim_player_set_src_data()、anim_player_get_segment()、anim_player_set_segment()等API进行设置，
                 *        所以需要其他task根据动画需求进行设置。
                 */
                ...
                if (format == IMAGE_FORMAT_INVALID) {
                    ...
                    //实际走这里进行正常播放，需要播放的数据信息，比如播放的帧数据都通过 header 指针可以索引到
...
esp_err_t anim_player_set_src_data(anim_player_handle_t handle, const void *src_data, size_t src_len)
{
    ...
    ESP_LOGI(TAG, "HELLO!! tets info.start:%d, info.end:%d", ctx->info.start, ctx->info.end);
    //default segment
    ctx->run_start = ctx->info.start;
    ctx->run_end = ctx->info.end;
    ctx->repeat = true;
    ctx->fps = CONFIG_PRIVATE_ANIM_PLAYER_DEFAULT_FPS;
...
void anim_player_set_segment(anim_player_handle_t handle, uint32_t start, uint32_t end, uint32_t fps, bool repeat)
{
    ...
    ctx->repeat = repeat;
    ctx->fps = fps;
    ESP_LOGI(TAG, "set segment: %" PRIu32 " -> %" PRIu32 ", repeat:%d, fps:%" PRIu32 "", start, end, repeat, fps);
}
...
anim_player_handle_t anim_player_init(const anim_player_config_t *config)
{
    ...
    player->run_end = 0;
    player->repeat = false;
    player->fps = CONFIG_PRIVATE_ANIM_PLAYER_DEFAULT_FPS;
    ...
    if (config->task.task_affinity < 0) {
        /**
         * @param   player 是作为anim_player_task()函数的入参用的
         */
